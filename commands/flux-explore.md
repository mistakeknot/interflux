...
4. **If `TEAMS_REQUESTED` is `true` AND fallback fired** (detection failed, cluster too close, runtime failure, or quality check failed): Step 4b ran the legacy synthesis and is producing its frontmatter. Add `teams_fallback: <reason>` after the `date` key. The reason MUST be exactly one of: `detection_disabled`, `detection_version`, `detection_unparseable`, `runtime_failure`, `divergent_clusters`, `synthesis_quality_check_fail`. Do NOT include `teams_used` or any Step 4c-only key in this case.
