...
Speculative launches do NOT count against the slot ceiling — they are bonus agents justified by Stage 1 evidence. Mark speculative launches in the triage report: `[speculative — launched after {agent} completed]`.

**Cap:** Maximum 2 speculative launches during Stage 1. If more candidates qualify, wait for the full expansion decision (Step 2.2b).
...
After Stage 1 completes (and AgentDropout filtering), read the Findings Index from each Stage 1 output file. Then use the **expansion scoring algorithm** to recommend which Stage 2 agents (and expansion pool agents not dropped by AgentDropout, and not already speculatively launched in Step 2.2a.6) to launch.
...
For each Stage 2 / expansion pool agent, compute an expansion score (note: Project Agents without adjacency can only reach expansion_score = 1, from domain criteria alone):
